...
_share_. A shared bit cannot be chosen per channel — declaring `[6, 6, 6]` would
tell the fit it can pick colours no palette word expresses — so five bits is the
_independently choosable_ precision and the honest lattice. That is now stated as
...
step _above_ black. So the sources fix the bit's position and not its sense, and
