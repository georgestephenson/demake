...
cell to one cell. This console's playfield is built from _sprite_ tiles and the
...
because the fix layer is 8×8 at 40×28, which _is_ the language's own cell grid:
...
that the _background_ has the same problem, which that console does not have
