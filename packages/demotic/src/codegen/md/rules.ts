...
function emitButton(ctx: MdCtx, set: number, action: string, skip: string): void {
...
function emitSnapshot(ctx: MdCtx, control: ControlDef, bind: Binding, base: number): void {
  const { asm, layout } = ctx;
  for (const [index, assignment] of control.assignments.entries()) {
    ...
    asm.move("b", eaImm(1), at(layout.holdFlags + base + index));
  }
}

function emitRestore(ctx: MdCtx, control: ControlDef, bind: Binding, base: number): void {
  const { asm, layout } = ctx;
  for (const [index, assignment] of control.assignments.entries()) {
    const target = assignment.target;
    if (target.kind !== "prop") continue;
    ...
    ctx.far("eq", skip);
    asm.clr("b", at(layout.holdFlags + base + index));
    const slot = layout.holdValues + (base + index) * 4;
