...
function emitButton(ctx: WscCtx, set: number, action: string, skip: string): void {
...
function emitSnapshot(ctx: WscCtx, control: ControlDef, bind: Binding, base: number): void {
  const { layout } = ctx;
  for (const [index, assignment] of control.assignments.entries()) {
    ...
    setByte(ctx, layout.holdFlags + base + index, 1);
  }
}

function emitRestore(ctx: WscCtx, control: ControlDef, bind: Binding, base: number): void {
  const { asm, layout } = ctx;
  for (const [index, assignment] of control.assignments.entries()) {
    const target = assignment.target;
    if (target.kind !== "prop") continue;
    ...
    ctx.far("z", skip);
    setByte(ctx, layout.holdFlags + base + index, 0);
    const slot = layout.holdValues + (base + index) * 4;
