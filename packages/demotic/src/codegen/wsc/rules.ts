/**
 * The tick, compiled for the V30MZ.
 *
 * `sim.ts` is the specification and this is a conformance implementation of it,
 * exactly as every other backend is. The *order* of the steps is not here at all
 * — `emitTickSteps` runs them (see `codegen/backend.ts`) — and every decision
 * about which rule can fire where is `shape.ts`'s. What is left is this machine's
 * instructions, and three places where they are shaped differently:
 *
 *   - **A box is staged by `rep movsw`.** The overlap test and the separation
 *     work on two fixed addresses, as on every console, so that they can be
 ...
 *     which is `si`, so a staging call is one `mov` and a `call`.
 *   - **A cheap rejection reads the other object through `bx`.** `NearBox`
 ...
 *     the reverse. The margins are compile-time constants and arrive in `cx`.
 *   - **A table is read with a segment override and its index is not.** Every
 *     table a loop walks — the entity addresses, the contact byte and mask — is
 *     cartridge data, so it is `cs:[bx+si]`; the cursor that indexes it is work
 *     RAM and is not. Getting that backwards reads a game's own variables as
 *     though they were a table of addresses.
 *
 * One thing is *simpler* than on the 8-bit consoles and worth saying: a
 * predicate routine can answer in the zero flag without ceremony, because `or
 * al, al` is one instruction and every arithmetic instruction here sets flags.
 */
...
/** Set the flags from a byte in memory, without loading it anywhere. */
function loadByte(ctx: WscCtx, address: Ref): void {
  ctx.asm.aluMI8("cmp", abs(address), 0);
}

/** Store a small constant into a byte of state. */
function setByte(ctx: WscCtx, address: Ref, value: number): void {
  ctx.asm.movmi8(abs(address), value);
}
 ...
 */
function guardVisible(ctx: WscCtx, id: number, skip: string): "always" | "never" | "runtime" {
 ...
 */
function guardVisiblePtr(ctx: WscCtx, skip: string): void {
  ctx.scoped(() => {
 ...
 */
function emitFire(ctx: WscCtx, rule: RuleDef, bind: Binding, trigger?: Trigger): void {
  const { asm } = ctx;
 ...
 * on exactly the tick the interpreter says it does. This console has no audio
 * driver yet, so what survives is the *record* of the request — which is a field
 * of the trace, and therefore has to be written whether anything is listening or
 * not (doc 14 §Conformance).
 */
export function emitSound(ctx: WscCtx, rule: RuleDef): void {
 ...
 */
function emitHoldEdges(ctx: WscCtx, scene: SceneCtx): void {
...
export function emitLevelRules(ctx: WscCtx, scene: SceneCtx): void {
  const { asm } = ctx;
 ...
 * {@link emitMoveLoop}).
 */
function moveShape(ctx: WscCtx, id: number): string {
  const instance = ctx.program.instances[id] as InstanceDef;
...
export function emitIntegrate(ctx: WscCtx, scene: SceneCtx): void {
  const { asm } = ctx;
 ...
 * the same body. They go through the pair loop's pointer instead, and by now
 * nothing here knows or cares whether the record it is stepping belongs to an
 * instance the compiler named.
 *
 ...
 */
function emitMoveLoop(ctx: WscCtx, ids: readonly number[]): boolean {
  const { asm } = ctx;
 ...
 */
function openProp(ctx: WscCtx, entity: EntityAddr, prop: string): { addr: Ref; close: () => void } {
 ...
 * may use every register there is.
 *
 * The table itself is in the cartridge, so the entry is read through `cs:` while
 * the cursor that indexes it is not.
 *
 * Returns the label the matching {@link emitLoopNext} branches back to.
 */
function emitLoopHead(ctx: WscCtx, table: string): string {
  const { asm, layout } = ctx;
  const ptr = layout.loop as number;
  const loop = ctx.unique("walkLoop");
  setByte(ctx, ptr + 2, 0);
  asm.label(loop);
  asm.movm8("bl", abs(ptr + 2));
  asm.movi8("bh", 0);
  asm.shift("shl", "bx");
  asm.movm("bx", romAt("bx", label(table)));
  asm.movmr(abs(ptr), "bx");
  return loop;
}

/** Step the cursor and go round again while entries remain. */
function emitLoopNext(ctx: WscCtx, loop: string, count: number): void {
  const { asm, layout } = ctx;
  const ptr = layout.loop as number;
  asm.incM8(abs(ptr + 2));
  asm.aluMI8("cmp", abs(ptr + 2), count);
  ctx.far("b", loop);
}

/** The addresses a loop walks, emitted after the code with the other tables. */
function emitEntityTable(ctx: WscCtx, table: string, bases: readonly number[]): void {
  ctx.data((data) => {
    data.label(table);
...
/**
 * `bx` = the contact byte's offset and `dl` = its mask, for the current entry.
 *
 ...
 * working a bit number out at run time would cost a shift loop per entry. Both
 * tables are addressed as `cs:[bx+si]` and `cs:[bx+di]`, which is the one place
 * this backend uses a base-plus-index operand.
 */
function needContactSlot(ctx: WscCtx): Ref {
  return ctx.need("ContactSlot", (inner) => {
    const { asm, layout } = inner;
    asm.movm8("bl", abs((layout.loop as number) + 2));
    asm.movi8("bh", 0);
    asm.movm8("al", romAt("bx+si"));
...
function emitContactBitPtr(
  ...
  asm.aluMR8("or", at("bx", layout.contacts), "dl");
}

/** The byte-and-mask tables one contact bit per entry needs. */
function emitContactTables(ctx: WscCtx, table: string, bits: readonly number[], suffix = ""): void {
  ctx.data((data) => {
...
function emitEdgeSeparate(ctx: WscCtx, entity: EntityAddr, edge: Edge, scene: SceneCtx): void {
 ...
 *
 * A load and a call: the source is a different entity each time, and `si` is
 ...
 * of eight bytes cost less than a second argument at every one of a game's
 * collision pairs.
 */
...
function needCopyBox(ctx: WscCtx, slot: "a" | "b"): Ref {
 ...
 * The answer is a flag rather than a byte so that a caller cannot forget to make
 * one out of it.
 */
function needOverlapPair(ctx: WscCtx): Ref {
 ...
 */
function needSeparatePair(ctx: WscCtx): Ref {
 ...
 */
function emitPairPushes(ctx: WscCtx, useY: string): { xPush: number; yPush: number } {
  const { asm, layout } = ctx;
  const a = layout.pairA as number;
  const b = layout.pairB as number;
...
/**
 * `al` = the {@link SIDE_BITS} bit for the side the staged pair sits on.
 *
 ...
 */
function needContactSide(ctx: WscCtx): Ref {
  return ctx.need("ContactSide", (inner) => {
    ...
    // The sign of a 16.16 value is the sign of its high half, which reaches the
    // flags by comparing that half against zero rather than by loading it.
...
/**
 * `bx` = the other object's base, `cl`/`ch` = the margins in cells → Z set when
 * the two boxes are certainly apart.
 *
 ...
 */
function needNearBox(ctx: WscCtx): Ref {
  return ctx.need("NearBox", (inner) => {
    ...
    const axis = (offset: number, margin: "cl" | "ch"): void => {
      // delta = other.cell - subject.cell, sixteen bits.
      asm.movm("ax", at("bx", offset + 2));
      asm.aluM("sub", "ax", abs(subject + offset + 2));
    ...
    axis(propOffset("x"), "cl");
    axis(propOffset("y"), "ch");
...
function emitCommitPair(ctx: WscCtx, entity: number): void {
  const { asm, layout } = ctx;
...
function emitContactSeen(ctx: WscCtx, bit: number, seen: string): void {
  ctx.asm.testMI8(abs(ctx.layout.contactsPrev + (bit >> 3)), 1 << (bit & 7));
  ctx.far("nz", seen);
}

function emitContactSet(ctx: WscCtx, bit: number): void {
  ctx.asm.aluMI8("or", abs(ctx.layout.contacts + (bit >> 3)), 1 << (bit & 7));
}
 ...
 * staging, the overlap, the rule body, the separation, the contact bit — is
 * hundreds of bytes. So the other object's record goes in the loop pointer and
 * the body is emitted once. `EntityAddr` has had a `ptr` case since the interface
 * was written and `expr.ts` implements it, so a rule body needs no special
 * handling at all.
 *
...
export function emitCollisions(ctx: WscCtx, scene: SceneCtx): void {
  const { asm, layout } = ctx;
  for (const rule of ctx.program.rules) {
...
export function emitEdgeRules(ctx: WscCtx, scene: SceneCtx): void {
  const { asm, layout } = ctx;
  for (const rule of ctx.program.rules) {
    ...
    if (rule.event.kind === "input") {
      ...
      if (rule.guard === undefined) {
        ...
        const flag = layout.scratch + S.w0;
        asm.testMI8(abs(set), 1 << bit);
        ctx.far("z", notFired);
        setByte(ctx, flag, 1);
        asm.jmp(fired);
        ...
        asm.label(fired);
        for (const subject of subjectBindings(ctx, rule, scene)) {
          emitSubjectFire(ctx, rule, subject, flag);
        }
      }
      continue;
    }
 ...
 */
function emitReaches(ctx: WscCtx, rule: RuleDef, scene: SceneCtx): void {
  const { asm, layout } = ctx;
  ...
  asm.movm8("al", abs(mem(delta, 3)));
  asm.aluM8("xor", "al", abs(mem(previous, 3)));
  asm.testI8("al", 0x80);
  ctx.far("z", notFired);
  asm.label(fired);
  setByte(ctx, firedFlag, 1);
  const run = ctx.unique("reachRun");
  asm.jmp(run);
  asm.label(notFired);
  setByte(ctx, firedFlag, 0);
  asm.label(run);
 ...
 */
export function emitCamera(ctx: WscCtx, scene: SceneCtx): void {
