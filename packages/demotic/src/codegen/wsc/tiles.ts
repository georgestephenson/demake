 ...
 */
export function emitTileSide(ctx: WscCtx, base: number): void {
  const { asm } = ctx;
  ctx.scoped(() => {
 ...
 */
function emitTilePushes(ctx: WscCtx, base: number, useY: string): { pushX: number; pushY: number } {
  const { asm, layout } = ctx;
  const col = layout.words + W.tileCol * 2;
  const row = layout.words + W.tileRow * 2;
