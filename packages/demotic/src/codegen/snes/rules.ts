...
function emitButton(ctx: SnesCtx, set: number, action: string, skip: string): void {
...
function emitSnapshot(ctx: SnesCtx, control: ControlDef, bind: Binding, base: number): void {
  const { layout } = ctx;
...
function emitRestore(ctx: SnesCtx, control: ControlDef, bind: Binding, base: number): void {
  const { asm, layout } = ctx;
  for (const [index, assignment] of control.assignments.entries()) {
    const target = assignment.target;
    if (target.kind !== "prop") continue;
    ...
    asm.lda(mem(layout.holdFlags + base + index));
