...
/**
 * A packed name table: literals and runs of whole *cells*.
 *
 * The encoder is `codegen/pack.ts`'s, because two consoles want it — a Sega
 * name-table entry is a tile and an attribute byte, and a PC Engine BAT entry is
 * a character and a sub-palette in one word, and both are two bytes a cell. It is
 * re-exported here because that is where this backend's readers look for it and
 * where `sms-rom.test.ts` imports it from.
 */
