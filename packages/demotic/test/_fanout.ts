...
/**
 * The Game Boy Advance has no battery, and the reason is a number rather than a
 * judgement.
 *
 * Its art path shares a bank out max-min fair on demands read off a first pass,
 * exactly as the Mega Drive's and the NES's do, so on the face of it it deserves
 * one. What it costs is the problem: a fit for this console is 38,400 pixels
 * against **252 centroids**, and a k-means iteration is O(N·K) — so one picture
 * is minutes where a Mega Drive's 320×224 into sixteen colours is seconds, and
 * one case would be around eight minutes on its own. A file that long is also a
 * worker nothing else can use, which is the whole reason these are per-console
 * files (§Why this is a battery rather than a test file).
 *
 ...
 * console with a backend. Adding it is worth doing the day a 256-colour fit is
 * cheaper; skipping it silently would not be.
 */
