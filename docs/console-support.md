...
| Mega Duck / Cougar Boy | `megaduck` | `gb` | yes | `bin` `asm` `c` | RGBDS | SameDuck | `gb` | yes | — | 120 Hz |
...
| Virtual Boy | `vb` | `vb` | yes | `bin` `asm` `c` | none (demake's own V810 assembler) | beetle-vb | `vb` | yes | yes | — |
...
- **18** demake music and sound effects; **15** play it from inside a game, and **12** build a cartridge whose only job is one schedule.
