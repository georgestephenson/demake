...
| Family | Runtime CPU | Assembled by | State |
|---|---|---|---|
| `gb` | SM83 | RGBDS — already provisioned | **written** |
| `nes` | 6502 | cc65 — already provisioned | planned |
| `sms` | Z80 (SMS + GG) | WLA-DX — already provisioned | planned |
| `md` | 68000 | GNU m68k binutils — already provisioned | planned |
| `snes` | 65816 | ca65 `--cpu 65816` — already provisioned | planned |
...
only the tables change per game, each runtime is assembled once and shipped as a
blob; the browser patches tables and prepped tiles into it and fixes the header
checksums. That generalises in-browser ROM building from "the two families where
writing a TS assembler was plausible" to *every family with a runtime* (doc 07).

The `gb` implementation of that is live, and settled two details the design left
open. The blob is **checked in rather than produced by CI**, generated by
`pnpm gen:runtime` and guarded by a staleness test in the same shape the man
pages use — a CI artifact would have made `demake build` depend on a build
having run, which is exactly the dependency the patching approach exists to
remove. And the blob reserves the game's window (`ds` at a fixed address) so
that the patch is a `set`, not a splice: no lengths move, and the only header
fields that change are the two checksums.

The Nintendo boot logo stays zero in the blob, on the same principle as the NDS
builder: we ship no copyrighted data. A patched ROM therefore direct-boots in
emulators and does not boot on original hardware; `demake build --boot-logo`
asks `rgbfix` to stamp it, which is the one optional step that wants a
toolchain.
