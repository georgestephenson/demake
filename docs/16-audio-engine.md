...
| Neo Geo | YM2610 | 4 FM + 3 SSG square + 6 ADPCM-A + 1 ADPCM-B | **Built** — fourteen voices, the widest single chip here, and four sections that have nothing to do with each other. Its FM section *is* the YM2612's core: the `$30`-`$B6` block is addressed at per-channel offsets **1 and 2** with offset 0 absent, `$28` names the four channels `001`, `010`, `101`, `110`, and the sample rate is 8 MHz over 144 — a six-channel part with two removed, so the model composes `Ym2612` and the binding calls the shared FM encoding. The six **ADPCM-A** voices are the first percussion in the matrix that is a *recording*: fixed at 18518.5 Hz with no register that would change it, so a drum is the only thing they can usefully be, and the bank is two ROMs in two codecs because ADPCM-A wraps a twelve-bit accumulator while ADPCM-B clamps a sixteen-bit one and scales its step by a multiplier. **Nothing is shared**, for two reasons at once — the SSG mixer is written once at boot because a note is silenced by its own level, and the ADPCM key-on is a *pulse* — so no merge routine is emitted. And the **clock is a timer and only a timer**, because this is the only console here whose sound processor cannot see the picture: the vertical blank reaches the 68000 alone. Still to come is the Z80 sound program, which is what would put a schedule inside a cartridge |
